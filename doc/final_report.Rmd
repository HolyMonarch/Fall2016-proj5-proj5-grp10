...
##Dataset Cleaning
The data set originally contained 20 attributes. In particular, Breed and Color contained over 3000 unique factors. After refactoring these columns, we have 36 types of dogs along with 30 different colors. Please reference "../lib/simplify_breeds.R". 
...
##Analysis
We started to look at the frequency of the outcome.

```{r Pie Chart of Outcome}
...
o_piechart
```
As shown above, the largest four components are adoption, transfer, return to owner, and euthanasia. Because the other components make up a small component, we set a primary forcus on the others. 


##Dataset vs.Outcome 
####Seasonality
To start, we looked at the seasonality of the animals in the shelter. 
...
```{r seasonality graph, echo=FALSE}
...
Seasonality
```
From the graph, we noticed that there seems to be some seasonality between the animals. However, when we furthered divided the animals we noticed there is a large seasonlity component for cats. This is not present for dogs. We can conclude there is very little seasonality for dogs. 

####Days in shelter VS. Outcome
We also anaylized how the number of days in the shelter affected the outcome of the dog. 

```{r shelter days, echo = FALSE}
...
shelter_days
```
From the graph, we can note that majority of the dogs stay for less than 10 days. For dogs who return to owners, they are more quickly returned than dogs who are adopted. 
/
We then analyzed if a dog has a name, how it will affect its outcome. 

```{r names vs. outcome, echo = FALSE}
...
```
As we can see, there seem to be a higher rate of being returned to owner if the dog has a name. There is also a significant increase for a dog to be transferred if it has no name. The adoption rate, however remains the same. 

####Sex Vs. Outcome
We then noticed that the gender of the dog is divided into four categories: Neutered Male, Spayed Female, Intact Female, and Intact Male. 

```{r Sex vs outcome, echo = FALSE}
#Deal with the data
...
```
For the dogs being returned to the owner, we noticed there is a larger spike for a specific dog to be a neutered male. All other categories remain the same. 
...
We noticed that hound dog seem to have the highest rate of adoption. 
```{r, echo = FALSE}
...
####Networking
We furthered wanted to see the path a dog will take once it enters the shelter. 
```{r sankey network, echo = FALSE}
#Sankey network
#Create the data
